...
function creatFileFuncByHtml(dirsArrContainer) {
    ...
    _.each(_.uniq(_.pluck(dirsArrContainer, 'outputFilePath')), function (value) {
        ...
        dirsFileArrContainer.push({
            writeFileSyncPath: path.join(value, 'index_jsdoc_zero_menu.html'),
            render           : {
                locals  : {
                    name   : _.str.strRight(path.join(value, 'index_jsdoc_zero_menu.html'), dirsArrContainer[0].outputPath),
                    content: {
                        ...
                        dirList : _.without(
                            _.map(fs.readdirSync(value), function (dirIndexItem) {
                                if (fs.statSync(path.join(value, dirIndexItem)).isDirectory()) return dirIndexItem
                            }),
                            undefined
                        )  //==> [ 'cc' ]
                    }
                },
                filename: path.join(_.str.strRight(value, dirsArrContainer[0].outputPath), 'index_jsdoc_zero_menu.html')
            }
        });
    });
                ...
                function (memo, num) {
                    ...
                    fs.writeFileSync(
                        ...
                        swig.render(
                            ...
                            {
                                locals  : {
                                    name   : _.str.strRight(pathChildrenCache, projectHomePath) + '/index_jsdoc_zero_menu.html', // 这里的 ‘/’ 不做 win 处理
                                    content: {
                                        outPath: [],   //==>/doc/dox
                                        dirList: [] //==> [ 'cc' ]
                                    }
                                },
                                filename: _.str.strRight(pathChildrenCache, projectHomePath) + '/index_jsdoc_zero_menu.html' // 这里的 ‘/’ 不做 win 处理
                            }
                        ),
                        'utf8');
    ...
    fs.writeFileSync(
        ...
        swig.render(
            getTemplates.indexZero,
            {
                ...
                filename: _.str.strRight(outputPathCompletion, projectHomePath) + '/index_jsdoc_zero_menu.html' // 这里的 ‘/’ 不做 win 处理
            }
        ),
        'utf8');
