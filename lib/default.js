...
function walk(pathIndexCompletion, excludeOutputPathCompletion, loadConfigFiled, callback) {
    ...
    if (!fs.existsSync(outputPathCache)) {
        ...
        if (_.indexOf(excludeOutputPathCompletion, outputPathCache) === -1) {
            ...
            console.log('开始生成子文件夹主页面空索引 [' + path.join(outputPathCache, 'index_jsdoc_zero_menu.html') + ']');
            fs.writeFileSync(
                path.join(outputPathCache, 'index_jsdoc_zero_menu.html'),
                swig.render(
                ...
                'utf8');
            console.log('----生成子文件夹主页面空索引 [' + path.join(outputPathCache, 'index_jsdoc_zero_menu.html') + '] 完成');
        }
    }
    /**
...
function creatFileFuncByHtml(dirsArrContainer) {
    ...
    _.each(_.uniq(_.pluck(dirsArrContainer, 'outputFilePath')), function (value) {
        ...
        console.log('覆盖子文件夹主页面空索引 [' + path.join(value, 'index_jsdoc_zero_menu.html') + ']');
        fs.writeFileSync(
            value + '/index_jsdoc_zero_menu.html',
            swig.render(
                getTemplates.pageList,
                {
                    locals  : {
            ...
            'utf8');
        console.log('----覆盖子文件夹主页面索引 [' + path.join(value, 'index_jsdoc_zero_menu.html') + '] 完成');
    });
